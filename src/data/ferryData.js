...
const generateDummyVesselData = () => {
  ...
  const centerLat = 1.258; // Moved to water area south of Marina Bay
  const centerLng = 103.86; // In the water between Marina Bay and Sentosa
  ...
  const startTime = new Date(
    now.getTime() - (timeWindowMinutes / 2) * 60 * 1000
  ); // 10 minutes ago
  ...
  for (let i = 0; i < totalPoints; i++) {
    ...
    const localTimeStr = currentTime
      .toISOString()
      .slice(0, 19)
      .replace("T", " ");
    const utcTimeStr =
    ...
    csvData += `${localTimeStr},${utcTimeStr},563072390,,MSF HAPPY,9V6734,Passenger ship,18,6,2.1,${sogKnots},${cogDegrees},${heading},${lat.toFixed(
      6
    )},${lng.toFixed(
      6
    )},Marina South Pier,Under way using engine,Singapore,60,,4,14,3,3\n`;
  }
...
export const loadVesselMovementData = async (
  csvFilePath,
  forceDummy = true
) => {
  try {
    ...
    if (forceDummy) {
      ...
      if (typeof window !== "undefined" && window.fs?.readFile) {
        ...
        try {
          ...
          console.warn(
            "Could not fetch CSV file, using fallback dummy data:",
            fetchError
          );
    ...
    return new Promise((resolve, reject) => {
      Papa.parse(csvData, {
        ...
        complete: (results) => {
          ...
          vesselMovementData = results.data
            .map((row, index) => {
              ...
              if (
                ...
                try {
                  ...
                  if (
                    ...
                    parsedDate <= oneMonthFromNow
                  ) {
                    timestamp = parsedDate.toISOString();
                  } else {
                    console.warn(
                      "Parsed date seems unreasonable:",
                      parsedDate,
                      "from:",
                      timeStr
                    );
              ...
              const processedRow = {
                ...
                vesselCategory: categorizeVessel(
                  row["Ship Type"] || row.shiptype
                ),
              };
          ...
          console.log(
            `Loaded ${vesselMovementData.length} vessel movement records from CSV`
          );
...
const determineRoute = (destination, vesselName) => {
  ...
  if (dest.includes("lazarus") || dest.includes("lazarus island"))
    return "MSP-LAZARUS";
  if (dest.includes("st john") || dest.includes("st. john"))
    return "MSP-STJOHN";
  ...
  if (name.includes("ferry") || name.includes("passenger"))
    return "FERRY-ROUTE";
...
const categorizeVessel = (shipType) => {
  ...
  if (type.includes("pleasure") || type.includes("yacht"))
    return "Recreational";
...
export const getVesselHistory = (vesselId, hoursBack = 24) => {
  ...
  return vesselMovementData
    .filter(
      (record) =>
        record.vesselId === vesselId && new Date(record.timestamp) >= cutoffTime
    )
...
export const simulateVesselMovement = (vesselId, updateCallback) => {
  const vessel = getCurrentVesselPositions().find(
  ...
  const interval = setInterval(() => {
    ...
    const deltaLng =
      (distanceKm / (111.32 * Math.cos((vessel.latitude * Math.PI) / 180))) *
      Math.sin(headingRad);
...
export const analyzeVesselMovement = (vesselId, hoursBack = 24) => {
  ...
  return {
    ...
    averageHeading:
      headings.length > 0
        ? (headings.reduce((a, b) => a + b, 0) / headings.length).toFixed(0)
        : 0,
...
export const getVesselTrafficSummary = () => {
  ...
  currentVessels.forEach((vessel) => {
    // Count by category
    summary.byCategory[vessel.vesselCategory] =
      (summary.byCategory[vessel.vesselCategory] || 0) + 1;

    // Count by status
    summary.byStatus[vessel.status] =
      (summary.byStatus[vessel.status] || 0) + 1;
  ...
  summary.averageSpeed =
    speedCount > 0 ? (totalSpeed / speedCount).toFixed(2) : 0;
